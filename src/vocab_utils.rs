...
pub(crate) struct CountInfo {
    pub cnt: SortedTokenId,
    pub rank_range: SortedTokenRange,
}
...
pub(crate) fn sort_vocab_with_trie<I: Ord + Clone, T: AsRef<[I]>, V: IntoIterator<Item = T>>(
    ...
    let res = trie.get_root_state().dfs_travel(|event| {
        match event {
            TravelEvent::PushRoot(state) | TravelEvent::Push(state, _, _) => {
                ...
                cnt_info.rank_range.lower = tot_cnt;
                tot_cnt += cnt_info.cnt;
            }
            TravelEvent::Pop(state, _) => {
                let id = state.node_id;
                let cnt_info = &mut cnt_info_of_trie[id];
                cnt_info.rank_range.upper = tot_cnt;
            }
        }
        Ok::<_, Infallible>(())
    });
    ...
    let order = {
        ...
        order.sort_by_key(|&i| cnt_info_of_vocab[i as usize].rank_range.lower);
        order
    };
